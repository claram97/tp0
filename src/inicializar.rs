...
use std::path::{Path, PathBuf};
    ...
    fn create_file_at_path(input_path_str: &str) -> io::Result<File> {
        ...
        if input_path.is_absolute() {
            ...
            full_path = current_dir.join(input_path);
        }

        println!("output path: {}", full_path.to_string_lossy());

        // Crea el archivo en la ruta completa
...
pub fn run(args: Vec<String>) -> io::Result<()> {
    ...
    let mut output_file = create_file_at_path(&output_file_name)?;
